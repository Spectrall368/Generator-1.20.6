...
<#macro itemAttributeModifiers includeMeleeAttributes=false>
	...
	.add(Attributes.ATTACK_DAMAGE, new AttributeModifier(BASE_ATTACK_DAMAGE_UUID, "Tool modifier", ${data.damageVsEntity - 1}, AttributeModifier.Operation.ADD_VALUE), EquipmentSlotGroup.MAINHAND)
	.add(Attributes.ATTACK_SPEED, new AttributeModifier(BASE_ATTACK_SPEED_UUID, "Tool modifier", ${data.attackSpeed - 4}, AttributeModifier.Operation.ADD_VALUE), EquipmentSlotGroup.MAINHAND)
	</#if>
	<#list data.attributeModifiers as modifier>
	.add(${modifier.attribute}, new AttributeModifier(
			UUID.fromString("${w.getUUID(registryname + "_" + modifier?index)}"), "Tool modifier",
