...
public class ${name}Item extends <#if data.hasBannerPatterns()>BannerPattern<#elseif data.isMusicDisc>Record</#if>Item {
	...
	public ${name}Item() {
				...
				.attributes(ItemAttributeModifiers.builder()
					...
					.add(Attributes.ATTACK_SPEED, new AttributeModifier(BASE_ATTACK_SPEED_UUID, "Item modifier", ${data.attackSpeed - 4},
							AttributeModifier.Operation.ADD_VALUE), EquipmentSlotGroup.MAINHAND)
					</#if>
					<#list data.attributeModifiers as modifier>
					.add(${modifier.attribute}, new AttributeModifier(
							UUID.fromString("${w.getUUID(registryname + "_" + modifier?index)}"), "Item modifier",
